...
class TuningPhysics {
  static calculateNaturalFrequency(springRate, sprungMass) {
    // f = (1 / 2π) * √(k/m)
    // springRate in N/mm, sprungMass in kg
  ...
  static springRateFromFrequency(frequency, sprungMass) {
    ...
    return k / 1000; // Convert to N/mm
  }
  
  static calculateDamperRatio(compression, expansion) {
    // GT7 uses arbitrary units, convert to meaningful ratio
    // Typical racing dampers: 0.6-0.8 compression, 0.3-0.5 rebound
    return {
      compressionRatio: compression / 100,
      reboundRatio: expansion / 100
    };
  }
  
  static calculateTorqueToWeight(torque, weight) {
    return (torque / weight) * 100;
  }
  
  static calculatePowerToWeight(hp, weight) {
    return hp / weight;
  }
  
  static estimatePP(hp, weight, aero, grip) {
    // Simplified PP estimation formula
...
export default function GT7TuningLab() {
  const [vehicleDatabase, setVehicleDatabase] = useState(null);
  ...
  const loadVehicleDatabase = async () => {
    try {
      ...
      // Try to load from public/vehicles.json (generated by scraper)
      ...
      if (data.vehicles) {
        ...
        console.log(`✅ Loaded ${data.metadata?.total_vehicles || 0} vehicles`);
      } else {
      ...
      console.warn('⚠️ Could not load vehicle database, using fallback');
  ...
  const manufacturers = vehicleDatabase ? Object.keys(vehicleDatabase).sort() : [];
  const models = selectedManufacturer && vehicleDatabase 
    ? Object.keys(vehicleDatabase[selectedManufacturer]).sort() 
    : [];
  const carData = selectedManufacturer && selectedModel && vehicleDatabase
    ? vehicleDatabase[selectedManufacturer][selectedModel] 
    : null;
